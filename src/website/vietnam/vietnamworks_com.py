...
from core.config import (MAX_WEBSITE_THREADS)
class VietnamworksComWebsiteStrategy:
    ...
    def parse_items(self, data_list):
        if not data_list:
            ...
            return []
        try:
            thread_count = min(len(data_list),MAX_WEBSITE_THREADS)
            logger.info(
                "Starting parser using %d threads.",thread_count)
            with ThreadPoolExecutor(max_workers=thread_count) as executor:
                results = list(executor.map(self.get_items,data_list))
            logger.info("Finished parsing %d jobs.",len(results))
            return results
        except Exception:
    ...
    def get_items(self, job):
        ...
        for field_name, method_name in self.FIELD_PARSERS.items():
            ...
            try:
                ...
                self.log_parser_error(field_name,job)
    ...
    def get_value(self, job, key):
        ...
        if not isinstance(job_data, dict):
            logger.warning(
                "Invalid job_data while reading '%s'.",key)
            return None
